...
These files are:
  ...
  * ``helloworld.js`` : File where the source code of our first helloworld lambda will be. By default gordon creates a function called ``handler`` in this file.
  * ``settings.yml`` : Configuration related to this application. By default gordon registers a ``helloworld`` lambda the function within ``helloworld.js``.
...
Give it a look to ``firstapp/settings.yml`` and ``firstapp/helloworld.js`` files in order to get a better understanding of what gordon just created for you.
